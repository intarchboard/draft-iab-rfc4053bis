...
untilizing information communication, like a simple email reply, if
...
to ensure that a response is generated by the deadline if a respone is intended.
...
responsibility to decide about the nessecary level of consensus needed
for a certain response. This section adds additional considerstion
...
on a document from another organization or a specific technical questions
not addressed in existing RFCs, the chairs should seeks for group input.
...
Potenial follow-up liaison statements might be sent to provide a status update,
